...
const UserUpdate = () => {
  ...
  const fetchUser = async () => {
    try {
      ...
      setError(error);
      alertService.error("Có lỗi xảy ra khi lấy thông tin người dùng.");
    }
  };
  ...
  const handleSubmit = async (values, { setSubmitting, resetForm }) => {
    try {
      ...
      setUser(response.data);
      alertService.success("Người dùng đã được cập nhật.");
      navigate("/user");
      resetForm();
    } catch (error) {
      alertService.error("Có lỗi xảy ra khi cập nhật người dùng.");
      setError(error);
    } finally {
      setSubmitting(false);
    }
      ...
      <div className="flex items-center mb-4">
        ...
        <div className="flex-grow text-center">
          <h1 className="text-2xl font-bold">
            {isEditing ? "Chỉnh sửa thông tin người dùng" : "Thông tin người dùng"}
          </h1>
        </div>
